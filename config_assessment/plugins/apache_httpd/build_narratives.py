...
Corre depois do build_llm.py (Stage 1 + Stage 2):
    python3 -m config_assessment.plugins.apache_httpd.build_narratives \\
        --db ccss.db \\
        --model qwen2.5:14b \\
        [--dry-run]
...
from config_assessment.build.llm_client import StubLLMClient, make_client
...
def run_narratives(
    ...
    service_name = _SERVICE_NAMES.get(target, "Apache HTTP Server")
...
if __name__ == "__main__":
    ...
    print(f"\nNext step:")
    print(f"  sca scan /path/to/httpd.conf --report --format html")
