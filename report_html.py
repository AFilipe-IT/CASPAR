...
def generate_html(result, resolved=None):
    ...
    def mrow(key, val, vdesc, why):
        return (f'<tr><td>{key}</td>'
                ...
                f'<td><span class="m-why">{_e(why)}</span></td></tr>')
    ...
    def render_issue(g, idx):
        ...
        if prereqs or example:
            ...
            ex_s = f'<div class="block-title" style="margin-bottom:6px;margin-top:10px">Example</div><div class="example-box">{_nl2br(example)}</div>' if example else ""
        ...
        locs_html = "".join(f'<span class="loc-tag">{_e(c)}</span>' for c in contexts) if contexts else ""
    ...
    for chain in active_chains:
        ...
        chains_html += (
            ...
            f'<span class="amp">&#215;{chain.amplification}</span>{_badge(chain.amplified_score)}</div>' +
